package lk.ac.pdn.eng.attendance.data.prefs
...
/**
 * Encrypted key/value store for the configurable server base URL, persisted
 * session cookies, and the cached signed-in user. Falls back to plain
 * SharedPreferences if the Android Keystore is unavailable on the device.
 */
class SessionPrefs(context: Context) {
    ...
    var baseUrl: String
        get() = prefs.getString(KEY_BASE_URL, BuildConfig.DEFAULT_API_BASE)
            ?.trim()?.trimEnd('/')
            ?.ifEmpty { BuildConfig.DEFAULT_API_BASE } ?: BuildConfig.DEFAULT_API_BASE
        set(value) {
