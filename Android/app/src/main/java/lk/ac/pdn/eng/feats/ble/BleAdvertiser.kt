...
class BleAdvertiser(private val context: Context) {
    ...
    /**
     * True if this device can advertise as a BLE peripheral. Gated on the
     * advertiser actually existing — `isMultipleAdvertisementSupported` only
     * reports *concurrent* advertisement support and is false on some phones
     * that can still broadcast the single set we need.
     */
    fun isSupported(): Boolean {
        val bt = adapter ?: return false
        return bt.bluetoothLeAdvertiser != null
    }
